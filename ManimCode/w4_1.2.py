...
class RandomPulseGraph(Scene):
    def construct(self):
        ...
        # Animate the opacity of the edges asynchronously
        animations = []
        for edge in edge_list:
            delay = random.uniform(0, 1)  # Random delay between 0 and 2 seconds
            animations.append(
                self.pulsate_edge(graph.edges[edge], delay)
            )

        self.play(*animations)
        self.wait(2)

    def pulsate_edge(self, edge, delay):
        # Create a pulsating effect by animating the opacity
        return AnimationGroup(
            ApplyMethod(edge.set_opacity, 1),
            Wait(delay),
            ApplyMethod(edge.set_opacity, 0.2),
            rate_func=there_and_back,  # Makes the opacity animate back and forth
            run_time=2,  # Duration of the pulsating effect
        )

if __name__ == "__main__":
    scene = RandomPulseGraph()
    scene.render()
